...
## Skills in Scope (11 total)

All root-level skills with `SKILL.md` directly in `~/.hermes/skills/`:
- `daily-tech-briefing`, `devops-github-cli-first`, `dogfood`, `gemini-video`
- `hermes-devops`, `hermes-mlops-inference`, `hermes-mlops-training`
- `notebooklm`, `pdf`, `yuanbao`
- `repo-sync-workflow` (from `autonomous-ai-agents/repo-sync-workflow/`)
...
### Manual: Push a new skill to repo

```bash
# 1. Copy skill to ~/badass-skills/
cp -r ~/.hermes/skills/my-new-skill ~/badass-skills/

# 2. Update README in repo
cd ~/badass-skills
# (README auto-regenerated by sync script, but you can regenerate manually)
./scripts/update-readme.sh   # or regenerate via Python

# 3. Commit and push
cd ~/badass-skills
git add -A && git commit -m "feat: add my-new-skill" && git push origin main
```

## Manual: Pull a skill from repo to local

```bash
# 1. Copy from repo
cp -r ~/badass-skills/my-skill ~/.hermes/skills/

# 2. Update manifest hashes
python3 /home/openclaw/.openclaw/workspace/scripts/update_manifest.py my-skill
```
...
# Check local skills
ls ~/.hermes/skills/*/SKILL.md | wc -l  # should be 11
```
